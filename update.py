...
def reload_plugins(message):
    if is_approved(message, 'admin'):
        ...
        os.chdir("slackbot")
        os.chdir("plugins")
        message.send(str(subprocess.Popen('bash update.sh', cwd=os.getcwd(), shell=True).returncode))
        reload_self(message)
...
def reload_plugin(message, plugin):
    if is_approved(message, 'admin'):
        ...
        os.chdir("slackbot")
        os.chdir("plugins")
        message.send(str(subprocess.Popen('bash update.sh %s' % plugin, cwd=os.getcwd(), shell=True).returncode))
        time.sleep(3)
        reload_self(message)
